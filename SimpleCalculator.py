...
class SimpleCalculator:
    ...
    def __init__(self):
        ...
        button_informations = {
            ...
            "±": (5, 1, self.opposite), "0": (5, 2), ",": (5, 3), "=": (5, 4, self.eval)
        }
        #TODO you maybe want to put the command directly in that button_positions (rename the variable)
        #TODO delete total_label and total_expression and replace current_expression by the total or expression
        #TODO replace self.update_label() to self.update_label(text) ça économise des lignes
        for value, information in button_informations.items():
            ...
            button = tk.Button(self.buttons_frame, text=value, borderwidth=0, command=information[2] if len(information) > 2 else "")
